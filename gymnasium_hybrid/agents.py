...
class BaseAgent:
    ...
    def _step(self) -> None:
        ...
        self.x = np.clip(self.x, -1, 1).item()
        self.y = np.clip(self.y, -1, 1).item()
...
class MovingAgent(BaseAgent):
    ...
    def break_(self) -> None:
        self.speed = (
            0 if self.speed < self.break_value else self.speed - self.break_value
        )
        self._step()
...
class SlidingAgent(BaseAgent):
    ...
    def accelerate(self, value: float) -> None:
        ...
        speed = np.sqrt(
            value**2
            + self.speed**2
            + 2 * value * self.speed * np.cos(self.phi - self.theta)
        )
        angle = self.theta + np.arctan2(
            self.speed * np.sin(self.phi - self.theta),
...
class HardMoveAgent(BaseAgent):
    def __init__(self, break_value: float, delta_t: float, num_actuators: int = 4):
        ...
        self.meta_to_mask = list(
            product(*[list(range(2)) for _ in range(self.num_actuators)])
        )
    ...
    def move(self, move_direction_meta: int, move_distances: list) -> None:
        ...
        self.move_vector = np.array(
            [
                move_directions_mask[i]
                * move_distances[i]
                * np.array(
                    [
                        np.cos(i * 2 * np.pi / self.num_actuators),
                        np.sin(i * 2 * np.pi / self.num_actuators),
                    ]
                )
